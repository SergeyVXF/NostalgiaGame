Shader "Retro Shaders Pro/Retro Lit Outline"
{
    Properties
    {
		...
		[Header(Outline)]
		[Toggle] _USE_OUTLINE("Enable Outline", Float) = 0
		_OutlineWidth("Outline Width", Range(0.0, 0.1)) = 0.002
    ...
    SubShader
    {
		...
		// Outline Pass - должен быть первым
		Pass
		{
			Name "Outline"
			Tags
			...
			#pragma shader_feature_local _USE_OUTLINE_ON
			#pragma shader_feature_local _OUTLINEWIDTHINDEPENDENT_ON
			...
			{
				OutlineV2F o = (OutlineV2F)0;
				UNITY_SETUP_INSTANCE_ID(v);
				UNITY_TRANSFER_INSTANCE_ID(v, o);
				UNITY_INITIALIZE_VERTEX_OUTPUT_STEREO(o);

#if _USE_OUTLINE_ON
				// Расширяем вершины вдоль нормали в object space (точно как в OmniShade)
				float outlineWidth = _OutlineWidth;
				half objDepth = -TransformWorldToView(TransformObjectToWorld(float3(0, 0, 0))).z;
				
				#if _OUTLINEWIDTHINDEPENDENT_ON
				...
				v.positionOS.xyz += outlineWidth * normal;
				
				// Преобразуем в clip space
				o.positionCS = TransformObjectToHClip(v.positionOS.xyz);
				
				// Z offset для outline (как в OmniShade)
				float outlineOffset = _OutlineZPos / -100 / objDepth;
				#if !UNITY_REVERSED_Z
					outlineOffset = -outlineOffset;
				#endif
				o.positionCS.z += outlineOffset;
				
				o.color = _OutlineColor;
#else
				// Если outline выключен, просто передаем позицию без изменений
				o.positionCS = TransformObjectToHClip(v.positionOS.xyz);
			...
			float4 outlineFrag(OutlineV2F i) : SV_TARGET
			{
        ...
        Pass
        {
			...
			float4 frag(v2f i, float facing : VFACE) : SV_TARGET
			{
				...
				// Apply secondary lights (Cluster Light Loop rendering).
