...
public class playerWeaponSystem : MonoBehaviour
{
    ...
    public bool useMountPointForWeaponParent;
    public string mountPointNameForWeaponParent;
    ...
    public void checkEventsOnWeaponInput (bool shootAtKeyDown)
    {
        if (shootAtKeyDown) {
            if (weaponSettings.useEventOnInputDown) {
                weaponSettings.eventOnInputDown.Invoke ();
            }
        } else {
            if (weaponSettings.useEventOnInputUp) {
                weaponSettings.eventOnInputUp.Invoke ();
            }
        }
    }
    ...
    public void shootWeapon (bool isThirdPersonView, bool shootAtKeyDown, bool fullbodyAwarenessActive)
    {
        ...
        checkEventsOnWeaponInput (shootAtKeyDown);
                    ...
                    for (int l = 0; l < weaponSettings.projectilesPerShoot; l++) {
                        ...
                        if (weaponSettings.useRayCastShoot || weaponCrossingSurface) {
                            ...
                            if (Physics.Raycast (forwardPosition, forwardDirection, out hit, currentRaycastDistance, currentLayerToCheck)) {
                                bool playerColliderDetectedNotFoundResult = false;

                                if (checkForExtraCollidersOnWeaponOwnerIfDetected) {
                                    if (GKC_Utils.checkIfObjectIsOnCharacterExtraColliderList (hit.collider, playerControllerGameObject)) {
                                        playerColliderDetectedNotFoundResult = false;

                                        //print ("collider detected is from onwer");
                                    } else {
                                        playerColliderDetectedNotFoundResult = true;
                                    }
                                } else {
                                    if (hit.collider != playerCollider) {
                                        playerColliderDetectedNotFoundResult = true;
                                ...
                                if (playerColliderDetectedNotFoundResult) {
    ...
    public bool checkForExtraCollidersOnWeaponOwnerIfDetected;
    ...
    void setProjectileInfo ()
    {
        ...
        if (!projectileInfoAssigned) {
            ...
            newProjectileInfo.checkObjectsWithMultipleDamageReceiversEnabled = weaponSettings.checkObjectsWithMultipleDamageReceiversEnabled;
    ...
    public Texture getWeaponIConHUD ()
    {
        return weaponSettings.weaponIConHUD;
    }
    ...
    public void setWeaponIconHUD (Texture newTexture)
    {
        if (newTexture == null) {
            return;
        }

        weaponSettings.weaponIConHUD = newTexture;
    }

    public void setWeaponIcon (Texture newTexture)
    {
        if (newTexture == null) {
            return;
        }

        weaponSettings.weaponIcon = newTexture;
    }
    ...
    public void setWeaponIconHUDInEditor (Texture newTexture)
    {
        setWeaponIconHUD (newTexture);

        updateComponent ();
    }

    public void setWeaponIconInEditor (Texture newTexture)
    {
        setWeaponIcon (newTexture);
    ...
    public void setWeaponParent (Transform parent, Animator animatorToUse)
    {
        ...
        checkUpdateWeaponParent (animatorToUse);

        updateComponent ();
    }

    void checkUpdateWeaponParent (Animator animatorToUse)
    {
        ...
        if (useMountPointForWeaponParent) {
            if (playerControllerGameObject != null) {
                Transform newMountPointTransform = GKC_Utils.getMountPointTransformByName (mountPointNameForWeaponParent, playerControllerGameObject.transform);
    ...
    public void setWeaponSystemName (string newName)
    {
        ...
        if (IKWeaponManager != null) {
            IKWeaponManager.gameObject.name = newName;
        }
    ...
    {
        if (weaponSettings.ammoName == currentAmmoName) {

            weaponSettings.ammoName = currentAmmoName;
    ...
    public void updateComponent ()
    {
        ...
        GKC_Utils.updateDirtyScene ("Update Player Weapon System " + gameObject.name, gameObject);
    }
}