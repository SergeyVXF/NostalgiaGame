            ...
            for (int i = 0; i < manager.waypointList.Count; i++) {
                if (manager.waypointList [i].waypointTransform != null) {
                    ...
                    if (manager.useHandleForWaypoints) {
                        ...
                        var fmh_129_70_638979118401562146 = Quaternion.identity; newPoint = Handles.FreeMoveHandle (oldPoint, manager.handleRadius, new Vector3 (.25f, .25f, .25f), Handles.CircleHandleCap);
