...
public class SimpleMainMenu : MonoBehaviour
{
    ...
    void Awake()
    {
        Debug.Log("SimpleMainMenu Awake() вызван!");
    }
    
    void Start()
    {
        Debug.Log("SimpleMainMenu Start() вызван!");
        CreateMainMenu();
    }
    
    void CreateMainMenu()
    {
        Debug.Log("CreateMainMenu() начал выполнение");
        // Создаем Canvas если его нет
        Canvas canvas = FindObjectOfType<Canvas>();
        Debug.Log("Найден Canvas: " + (canvas != null ? canvas.name : "нет"));
        if (canvas == null)
        {
        ...
        {
            Debug.Log("Canvas уже существует, используем его");
        }
        
        Debug.Log("Начинаем создание панели меню");
    ...
    void CreateButton(GameObject parent, string text, System.Action onClick)
    {
        ...
        Debug.Log("Добавляем обработчик для кнопки: " + text);
        button.onClick.AddListener(() => {
            Debug.Log("Кнопка нажата: " + text);
            onClick?.Invoke();
        });
    }
