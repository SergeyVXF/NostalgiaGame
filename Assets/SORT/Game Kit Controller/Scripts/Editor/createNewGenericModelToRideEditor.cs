...
//		windowRect.position = new Vector2 (0, windowRect.position.y);
	...
	void addCharacter ()
	{
		...
		wolfCharacterController currentWolfCharacterController = newCustomCharacterObject.GetComponentInChildren<wolfCharacterController> ();

		if (currentWolfCharacterController != null) {
			currentWolfCharacterController.setCharacterPivotTransform (newCustomCharacterObject.transform);

            GKC_Utils.updateComponent (currentWolfCharacterController);
        }
