...
public class findObjectivesSystem : MonoBehaviour
{
    ...
    [Space]
    [Header ("AI Vehicle Behavior")]
    [Space]

    public AIBehaviorInfo vehicleAIBehavior;
    ...
    public bool loseTargetIfNotVisibileOnlyIfSpotted;
    ...
    public float groundHitPointOffset = 0.3f;
    ...
    public bool targetIsRiderObject;
    ...
    public mainRiderSystem currentRiderToAttack;
    ...
    float originalMinDistanceToEnemyUsingWeapons;
    ...
    bool lookDirectionToTargetOffsetActive;

    float currentLookDirectionToTargetOffset;

    bool usingDevicesManagerLocated;

    bool vehicleAIBehaviorChecked;

    bool vehicleAIBehaviorLocated;
    ...
    void Awake ()
    {
        ...
        originalMinDistanceToEnemyUsingWeapons = minDistanceToEnemyUsingWeapons;
        ...
        usingDevicesManagerLocated = usingDevicesManager != null;

        vehicleAIBehaviorLocated = vehicleAIBehavior != null;
    }
    ...
    void Update ()
    {
        ...
        updateAIPassengerBehaviorState ();
                                    ...
                                    if (!ignoreRaycastCheck) {
                                        if (Physics.Raycast (wonderTargetPosition, -Vector3Up, out hit, longDistance, layerMaskToRaycastToWander)) {
                                            ...
                                            if (AINavMeshManager.updateCurrentNavMeshPath (hit.point)) {
                                                ...
                                                emptyTargetPositionTransform.position = hit.point + groundHitPointOffset * hit.normal;
    ...
    public void disableStrafeModeIfActive ()
    {
        if (strafeCurrentlyActive) {
            playerControllerManager.activateOrDeactivateStrafeMode (false);
    ...
    public void checkCurrentThreat ()
    {
        ...
        if (possibleThreatDetected && posibleThreat != null) {
            ...
            if (Physics.Raycast (rayCastPosition.position, rayCastPosition.forward, out hit, longDistance, layerMask)) {
                bool useExtraRaycastResult = false;

                if (checkForExtraCollidersOnOwnerIfDetected) {
                    if (GKC_Utils.checkIfObjectIsOnCharacterExtraColliderList (hit.collider, playerControllerManager)) {
                        useExtraRaycastResult = true;

                        print ("collider detected is from onwer");
                    }
                }

                if (useExtraRaycastResult) {
                    if (Physics.Raycast (hit.point + 0.2f * rayCastPosition.forward, rayCastPosition.forward, out RaycastHit temporaryHit, longDistance, layerMask)) {
                        hit = temporaryHit;
                    }
                }
    ...
    public bool checkIfTargetVisible (float raycastDistance)
    {
        ...
        Vector3 newRaycastPosition = rayCastPosition.position;

        if (useCustomLayerOnTargetDetected) {
            raycastResult = Physics.Raycast (newRaycastPosition, raycastDirection, out hit, raycastDistance, customLayerOnTargetDetected);
        } else {
            raycastResult = Physics.Raycast (newRaycastPosition, raycastDirection, out hit, raycastDistance, layerMask);
        }
        ...
        if (useExtraRaycastResult) {
            newRaycastPosition = hit.point + 0.2f * targetToCheckDirection;

            if (useCustomLayerOnTargetDetected) {
                raycastResult = Physics.Raycast (hit.point + 0.2f * raycastDirection, raycastDirection, out hit, raycastDistance, customLayerOnTargetDetected);
            } else {
                raycastResult = Physics.Raycast (hit.point + 0.2f * raycastDirection, raycastDirection, out hit, raycastDistance, layerMask);
            }
        }
        ...
        if (raycastResult) {
            if (hit.collider.gameObject == targetToAttack || hit.collider.gameObject.transform.IsChildOf (targetToAttack.transform)) {
                ...
                if (showGizmo) {
                    Debug.DrawLine (newRaycastPosition, hit.collider.transform.position, Color.green);
                }
            ...
            if (showGizmo) {
                Debug.DrawRay (newRaycastPosition, 3 * rayCastPosition.forward, Color.black);
            }
        }
    ...
    void lookAtTarget (Transform objective)
    {
        ...
        if (objective != null) {
            ...
            if (lookDirectionToTargetOffsetActive) {
                Quaternion offsetRotation = Quaternion.Euler (AITransform.up * currentLookDirectionToTargetOffset);
    ...
    public void addLookDirectionToTargetOffset (float newValue)
    {
        currentLookDirectionToTargetOffset = newValue;

        lookDirectionToTargetOffsetActive = currentLookDirectionToTargetOffset != 0;
    }
        ...
        if (characterFactionToCheck != null) {
            ...
            if (currentVehicleHUDManager != null) {
                if (currentVehicleHUDManager.isVehicleBeingDriven ()) {
                    currentDriver = currentVehicleHUDManager.getCurrentDriver ();
                }
            } else {
                GameObject temporalRiderObject = applyDamage.getCharacterOrVehicle (character);

                if (temporalRiderObject != null) {
                    currentRiderSystem = applyDamage.getMainRiderSystem (temporalRiderObject);

                    if (currentRiderSystem != null) {
                        if (currentRiderSystem.isBeingDrivenActive ()) {
                            currentDriver = currentRiderSystem.getCurrentDriver ();
                        }
                    }
                }
            }
    ...
    void checkPossibleThreatValues (GameObject currentSuspect)
    {
        ...
        if (canCheckSuspect (currentSuspect.layer)) {
            if (!onSpotted && posibleThreat == null && checkCharacterFaction (currentSuspect, false)) {
                ...
                targetIsRiderObject = applyDamage.isRiderObject (currentSuspect);
    ...
    void addEnemy (GameObject enemyToAdd)
    {
        if (!enemies.Contains (enemyToAdd)) {
            ...
            newEnemyInfo.isRiderObject = applyDamage.isRiderObject (newEnemyInfo.enemyGameObject);

            if (newEnemyInfo.isRiderObject) {
                newEnemyInfo.isCharacter = false;
                newEnemyInfo.isVehicle = true;
            }
            ...
            if (newEnemyInfo.isCharacter) {
                ...
                float customCharacterRadius = applyDamage.getCustomCharacterRadius (newEnemyInfo.enemyGameObject);

                if (customCharacterRadius != -1) {
                    newEnemyInfo.characterRadius = customCharacterRadius;
                }
            }
    ...
    public void addNotEnemy (GameObject notEnemy)
    {
    ...
    void closestTarget ()
    {
        ...
        if (currentEnemiesCount > 0) {
            ...
            if (!currentEnemyInfoStored) {
                ...
                targetIsRiderObject = currentEnemyInfo.isRiderObject;
                ...
                if (targetIsVehicle) {
                    if (targetIsRiderObject) {
                        currentRiderToAttack = applyDamage.getMainRiderSystem (currentEnemyInfo.enemyGameObject);
                    } else {
                ...
                if (currentPlayerController != null && currentPlayerController.isPlayerDriving ()) {
                    ...
                    if (vehicleToCheck != null && !enemies.Contains (vehicleToCheck)) {
                        ...
                        if (temporalVehicleHUDManager != null) {
                            ...
                            GameObject temporalRiderObject = applyDamage.getCharacterOrVehicle (vehicleToCheck);

                            if (temporalRiderObject != null) {
                                currentRiderSystem = applyDamage.getMainRiderSystem (temporalRiderObject);

                                if (currentRiderSystem != null) {
                                    checkTriggerInfo (currentRiderSystem.getVehicleCollider (), true);
                                }
                            }
                        }
                    }
            ...
            if (targetIsVehicle) {
                ...
                if (targetIsRiderObject) {
                    if (!currentRiderToAttack.isBeingDrivenActive ()) {
            ...
            if (!currentEnemyInfoStored) {
                ...
                if (targetIsVehicle) {
                    float distanceToConfigure = 0;

                    if (targetIsRiderObject) {
                        distanceToConfigure = currentRiderToAttack.getVehicleRadius ();
                    } else {
                        distanceToConfigure = currentVehicleToAttack.getVehicleRadius ();
                    }
            ...
            if (!onSpotted) {
                ...
                if (seeingCurrentTarget) {
                    ...
                    if (useMaxTimeToLoseTargetIfNotVisible && !loseTargetIfNotVisibileOnlyIfSpotted && lastTimeEnemyVisible > 0) {
            ...
            if (onSpotted) {
                if (!currentEnemyInfoStored) {
                    ...
                    updateCurrentTargetSelectedInfo ();
                }
    ...
    public void updateCurrentTargetSelectedInfo ()
    {
        if (currentAIBehaviorAssigned) {
            currentAIBehavior.updateCurrentTargetSelectedInfo ();
        }
    }
    ...
    public bool checkIfTargetIsPhysicallyVisible (GameObject targetToCheck, bool checkingSuspect)
    {
        ...
        if (useCustomLayerOnTargetDetected && !checkingSuspect) {
            raycastResult = Physics.Raycast (newRaycastPosition, targetToCheckDirection, out hit, longDistance, customLayerOnTargetDetected);
        } else {
            raycastResult = Physics.Raycast (newRaycastPosition, targetToCheckDirection, out hit, longDistance, layerToCheckRaycastToViewTarget);
        }
        ...
        if (raycastResult) {
                ...
                if (temporalHealthManagementToCheck != null) {
                    ...
                    if (targetIsRiderObject) {
                        if (!targetVisibilityConfirmed) {
                            if (applyDamage.checkIfDetectSurfaceBelongToRider (targetToCheck, hit.collider)) {
                                targetVisibilityConfirmed = true;
                            }
                        }
                    } else {
            ...
            if (targetVisibilityConfirmed) {
                ...
                if (showGizmo) {
                    Debug.DrawRay (newRaycastPosition, targetToCheckDirection, Color.green, 2);
                }

                return true;
            } else {
                if (showGizmo) {
                    Debug.DrawRay (newRaycastPosition, targetToCheckDirection, Color.red, 2);
                }
            ...
            if (showGizmo) {
                Debug.DrawRay (newRaycastPosition, targetToCheckDirection, Color.black, 2);
            }
        }
    ...
    public void setBehaviorAttackEnabledState (bool state)
    {
        if (currentAIBehaviorAssigned) {
            currentAIBehavior.setAttackEnabledState (state);
        }
    }
    ...
    public void setAIStateToAlive ()
    {
        ...
        if (showDebugPrint) {
            print ("setAIStateToAlive " + AIDead + " " + AIPaused);
        }
        ...
        if (!AIDead) {
            if (showDebugPrint) {
                print (AIPatrolManagerLocated + " " + wanderEnabled);
            }

            if (AIPatrolManagerLocated && !wanderEnabled) {
                AIPatrolManager.pauseOrPlayPatrol (false);
            }
    ...
    {
        if (ignoreOnTriggerEnterAndExitOnMainAIObjectEnabled) {
            return;
        }
        ...
        if (canCheckSuspect (col.gameObject.layer)) {
            ...
            if (isEnter) {
                ...
                if (!AIPaused || targetDetectionActiveOnPausedAIState) {
                    ...
                    if (col.gameObject == AIGameObject) {
                        return;
                    }

                    if (playerControllerManager.isColliderPartOfCharacter (col)) {
                        return;
                    }
                    ...
                    if (checkCharacterFaction (col.gameObject, false)) {
                        ...
                        if (currentVehicleHUDManager != null) {
                            ...
                            if (currentRiderSystem != null) {
                                if (currentRiderSystem.isBeingDrivenActive ()) {
                                    GameObject currentDriver = currentRiderSystem.getCurrentDriver ();
                        ...
                        if (factionManager.checkIfCharacterBelongsToFaction (factionToFollowAsPartner, objectToCheck) && !partnerFound) {
                            if (!checkCharacterFaction (objectToCheck, false)) {
                                if (followPartnerOnTrigger) {
                                    ...
                                    setCurrentPlayerControllerPartner (partner.GetComponent<playerController> ());
    ...
    public void removePartner ()
    {
        ...
        setCurrentPlayerControllerPartner (null);
    ...
    void setCurrentPlayerControllerPartner (playerController newPlayerController)
    {
        playerControllerPartner = newPlayerController;

        if (vehicleAIBehaviorLocated) {
            vehicleAIBehavior.setCurrentPlayerControllerPartner (playerControllerPartner);
        }
    }
    ...
    public void setNewFireWeaponMinDistanceValues (float minDistanceToDrawValue, float minDistanceToAimValue, float minDistanceToShootValue)
    {
        minDistanceToDraw = minDistanceToDrawValue;
        minDistanceToAim = minDistanceToAimValue;
        minDistanceToShoot = minDistanceToShootValue;
    }
    ...
    public void setNewMinDistanceToEnemyUsingWeapons (float newValue)
    {
        minDistanceToEnemyUsingWeapons = newValue;

        if (showDebugPrint) {
            print ("setNewMinDistanceToEnemyUsingWeapons " + newValue);
        }

        AINavMeshManager.setDistanceToEnemy (minDistanceToEnemyUsingWeapons);
    }

    public void setOriginalMinDistanceToEnemyUsingWeapons ()
    {
        setNewMinDistanceToEnemyUsingWeapons (originalMinDistanceToEnemyUsingWeapons);
    }

    public float getOriginalMinDistanceToEnemyUsingWeapons ()
    {
        return originalMinDistanceToEnemyUsingWeapons;
    }

    public float getCurrentCharacterToAttackRadius ()
    {
        if (onSpotted) {
            if (targetIsCharacter || targetIsOtherType) {

                if (currentEnemyInfo != null) {
                    return currentEnemyInfo.characterRadius;
                }
            }

            if (targetIsVehicle) {
                if (targetIsRiderObject) {
                    if (currentRiderToAttack != null) {
                        return currentRiderToAttack.getVehicleRadius ();
                    }
                } else {
                    if (currentVehicleToAttack != null) {
                        return currentVehicleToAttack.getVehicleRadius ();
                    }
                }
            }
        }
    ...
    public void updateAIPassengerBehaviorState ()
    {
        if (!vehicleAIBehaviorChecked) {
            vehicleAIBehaviorLocated = vehicleAIBehavior != null;

            vehicleAIBehaviorChecked = true;
        }

        if (vehicleAIBehaviorLocated) {
            vehicleAIBehavior.updateAIPassengerBehavior ();
        }
    }
    ...
    {
        if (vehicleAIBehaviorLocated) {
            vehicleAIBehavior.getOffFromVehicle ();
        }
    }
    ...
    {
        if (vehicleAIBehaviorLocated) {
            vehicleAIBehavior.removeStateOnVehicle ();
        }
    }
    ...
    {
        if (vehicleAIBehaviorLocated) {
            vehicleAIBehavior.setAICharacterOnVehicle (newVehicle);
        }
    }
    ...
    public void setBehaviorAttackEnabledStateFromEditor (bool state)
    {
        if (mainWeaponAIBehavior != null) {
            mainWeaponAIBehavior.setAttackEnabledStateFromEditor (state);
        }

        if (mainPowerAIBehavior != null) {
            mainPowerAIBehavior.setAttackEnabledStateFromEditor (state);
        }

        if (mainCloseCombatAIBehavior != null) {
            mainCloseCombatAIBehavior.setAttackEnabledStateFromEditor (state);
        }

        if (mainMeleAIBehavior != null) {
            mainMeleAIBehavior.setAttackEnabledStateFromEditor (state);
        }
    }
