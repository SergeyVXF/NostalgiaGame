...
public class inventoryListManager : MonoBehaviour
{
    ...
    public fullCraftingBlueprintInfoTemplateData getFullCraftingBlueprintInfoTemplateData ()
    {
        return mainFullCraftingBlueprintInfoTemplateData;
    }
    ...
    public void createInventoryPrafab (int categoryIndex, int index)
    {
        ...
        if (prefabExists) {
            ...
            GKC_Utils.replacePrefabNameBased (newEmptyInventoryPrefab, prefab);

            print ("Replacing prefab in path " + prefabFilePath);
        } else {
            GKC_Utils.replacePrefabConnectToPrefab (newEmptyInventoryPrefab, prefabFilePath);
    ...
    public void updateInventoryPrefab (int categoryIndex, int index)
    {
        ...
        if (currentInventoryInfo.inventoryObjectPrefab != null) {
            ...
            if (currentInventoryObject != null) {
                ...
                if (settingNewInventoryInfoNameResult) {
                    currentInventoryObject.inventoryObjectInfo.setNewName (currentInventoryInfo.Name);
                }
                ...
                if (settingNewInventoryInfoNameResult) {
                    pickupType currentPickupType = currentInventoryInfo.inventoryObjectPrefab.GetComponentInChildren<pickupType> ();

                    if (currentPickupType != null) {
                        currentPickupType.renameObject (currentInventoryInfo.Name);
                    }
                }
    ...
    public void renameInventoryObject (string currentObjectName, string newObjectName)
    {
        inventoryInfo currentInventoryInfo = getInventoryInfoFromName (currentObjectName);

        if (currentInventoryInfo != null) {

            int categoryIndex = currentInventoryInfo.categoryIndex;
            int elementIndex = currentInventoryInfo.elementIndex;

            currentInventoryInfo = inventoryCategoryInfoList [categoryIndex].inventoryList [elementIndex];

            currentInventoryInfo.setNewName (newObjectName);

            updateInventoryPrefab (categoryIndex, elementIndex);

        }
    }

    public void renameCraftingObjectByName (string currentObjectName, string newObjectName)
    {
        if (mainFullCraftingBlueprintInfoTemplateData == null) {
            return;
        }
        ...
        for (int i = 0; i < dataListCount; i++) {
            ...
            for (int j = 0; j < craftingBlueprintInfoTemplateDataListCount; j++) {
                ...
                int currentIndex = craftingBlueprintInfoTemplateDataList [j].craftingBlueprintInfoTemplateList.FindIndex (s => s.Name.Equals (currentObjectName));

                if (currentIndex > -1) {
                    craftingBlueprintInfoTemplate currentBlueprint =

                    craftingBlueprintInfoTemplateDataList [j].craftingBlueprintInfoTemplateList [currentIndex];

                    currentBlueprint.Name = newObjectName;

                    print ("trying to convert to object " + currentBlueprint.Name);

                    UnityEngine.Object currentBlueprintObject = currentBlueprint as UnityEngine.Object;

                    if (currentBlueprintObject != null) {
                        print ("converted, renaming prefab");

                        GKC_Utils.renameAsset (currentBlueprintObject, newObjectName);
