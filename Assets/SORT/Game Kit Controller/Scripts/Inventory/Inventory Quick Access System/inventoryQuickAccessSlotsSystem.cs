...
public class inventoryQuickAccessSlotsSystem : MonoBehaviour
{
    ...
    public bool instantiateQuickSlotPrefabEnabled = true;

    public List<GameObject> quickAccessSlotsList = new List<GameObject> ();
    ...
    {
        GameObject newQuickAccessSlot = null;

        if (instantiateQuickSlotPrefabEnabled) {
            if (quickAccessSlotsList.Count == 0) {
                instantiateQuickSlotPrefabEnabled = false;
            } else {
                numberQuickAccessSlots = quickAccessSlotsList.Count;
            }
        }

        for (int i = 0; i < numberQuickAccessSlots; i++) {
            if (instantiateQuickSlotPrefabEnabled) {
                newQuickAccessSlot = quickAccessSlotsList [i];
            } else {
                newQuickAccessSlot = (GameObject)Instantiate (slotPrefab, Vector3.zero, Quaternion.identity, slotPrefab.transform.parent);
            }
                            ...
                            if (quickSlotFoundOnPress == slotElementFoundOnDrop.mainQuickAccessSlotInfo) {
                                ...
                                if (quickSlotFoundOnPress.inventoryInfoAssigned.isWeapon) {
                                    ...
                                    if (isMeleeWeapon) {
                                        ...
                                        mainMeleeWeaponsGrabbedManager.equipMeleeWeapon (currentObjectName, false, false);
                        ...
                        if (slotElementFoundOnDrop.mainQuickAccessSlotInfo.slotActive) {
                            if (currentSlotToMoveInventoryObject.isWeapon) {
                                ...
                                if (isMeleeWeapon) {
                                    ...
                                    mainMeleeWeaponsGrabbedManager.equipMeleeWeapon (currentSlotToMoveInventoryObject.Name, false, false);
