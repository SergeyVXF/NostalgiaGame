...
namespace Invector.vCharacterController
{
    ...
    public class vThirdPersonController : vThirdPersonAnimator
    {
        ...
        {
            var sprintConditions = (currentStamina > 0 && input.sqrMagnitude > 0.1f && !isCrouching && isGrounded && !actions && !(isStrafing && !strafeSpeed.walkByDefault && (direction >= 0.5 || direction <= -0.5 || speed <= 0)));
        ...
        private float _lastInputSqrMagnitude = 0f;
        private bool _footstepSoundsStopped = false;

        void Update()
        {
            // Проверяем, остановился ли игрок
            if (input.sqrMagnitude < 0.01f && !_footstepSoundsStopped)
            {
                StopAllFootstepSounds();
                _footstepSoundsStopped = true;
            }
            else if (input.sqrMagnitude >= 0.01f)
            {
                _footstepSoundsStopped = false;
            }
        }

        private void StopAllFootstepSounds()
        {
            var allFootstepAudios = GameObject.FindObjectsOfType<vAudioSurfaceControl>();
            foreach (var audio in allFootstepAudios)
            {
