    ...
    public abstract class vThirdPersonMotor : vCharacter
    {
        ...
        public virtual void FreeMovement()
        {
            ...
            if (input != Vector2.zero && targetDirection.magnitude > 0.1f && conditions && !lockRotation)
            {
                ...
                if (isGrounded || (!isGrounded && jumpAirControl))
                {
                    ...
                    if (animator.IsInTransition(0) && inTurn) return;
        ...
        protected virtual void CheckGroundDistance()
        {
            ...
            if (_capsuleCollider != null)
            {
                ...
                if (Physics.Raycast(ray2, out groundHit, colliderHeight / 2 + 2f, groundLayer) && !groundHit.collider.isTrigger)
                ...
                if (groundCheckMethod == GroundCheckMethod.High)
                {
                    ...
                    if (Physics.SphereCast(ray, radius, out groundHit, _capsuleCollider.radius + 2f, groundLayer) && !groundHit.collider.isTrigger)
                    {
        ...
        protected virtual void Sliding()
        {
            ...
            if (GroundAngle() >= slopeLimit + 1 && GroundAngle() <= 85 && groundDistance <= 0.05f)
            {
                ...
                var dir = _rigidbody.linearVelocity;
            }
            else
            {
                isSliding = false;
        ...
        {            
            ...
            Vector3 forward = Vector3.ProjectOnPlane(_movementDirection, groundHit.normal);         
            Ray rayStep = new Ray((transform.position + new Vector3(0, stepOffsetEnd, 0) + forward * ((_capsuleCollider).radius + 0.05f)), Vector3.down);

            if (Physics.Raycast(rayStep, out _hit, stepOffsetEnd - stepOffsetStart, groundLayer) && !_hit.collider.isTrigger)
            {
                if (_hit.point.y >= (transform.position.y+0.1f) && _hit.point.y <= (transform.position.y + stepOffsetEnd))
                {
                    ...
                    if (transform.position.y > _hit.point.y + 0.1f)
                        vel.y = 0;
                    _capsuleCollider.material = slippyPhysics;
                    _rigidbody.linearVelocity = vel;
                    return true;
