...
namespace Invector
{
    public class vFootStep : vFootPlantingPlayer
    {
        ...
        {
            Debug.Log($"[FootStep] StepOnTerrain вызван: {footStepObject.sender.name} на {footStepObject.name}");
        ...
        {
            Debug.Log($"[FootStep] StepOnMesh вызван: {footStepObject.sender.name} на {footStepObject.name}");
            if (currentStep != null && currentStep == footStepObject.sender) return;
            currentStep = footStepObject.sender;
            PlayFootFallSound(footStepObject, spawnParticle, spawnStepMark, volume);
            if (debugTextureName)
