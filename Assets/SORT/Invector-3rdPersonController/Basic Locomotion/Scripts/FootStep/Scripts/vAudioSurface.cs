...
namespace Invector
{
    public class vAudioSurface : ScriptableObject
    {
        ...
        [SerializeField] public List<AudioClip> audioClips;     // The different clips that can be played on this surface.    
        ...
        {
            Debug.Log($"[FootStep] PlayRandomClip вызван. Клип count: {(audioClips != null ? audioClips.Count : 0)}");
            ...
            Debug.Log($"[FootStep] PlayOneShot: {audioClips[index]?.name}");
        }
