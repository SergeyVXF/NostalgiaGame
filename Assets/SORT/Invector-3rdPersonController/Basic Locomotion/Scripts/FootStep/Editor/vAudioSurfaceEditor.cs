...
namespace Invector
{
    ...
    public class AudioSurfaceEditor : Editor
    {
        ...
        void DrawSimpleList(SerializedProperty list, bool useDraBox)
        {
            ...
            // Используем стандартный PropertyField для audioClips
            if (list.name == "audioClips")
            {
                EditorGUILayout.PropertyField(list, true); // true = отрисовать дочерние элементы (стрелка, +, drag&drop)
            }
            else
            {
