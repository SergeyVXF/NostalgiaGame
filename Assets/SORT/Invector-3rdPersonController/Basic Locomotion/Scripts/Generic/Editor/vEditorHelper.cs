...
namespace Invector
{
    ...
    public class vEditorBase : Editor
    {
            ...
            if (((vMonoBehaviour)target) != null)
            {
               ...
               var nullToolBar = toolbars.FindAll(tool => tool != null && (tool.variables == null || tool.variables.Count == 0));
                ...
                if (headerAttribute != null)
                    m_Logo = LoadIcon(headerAttribute.iconName);
        ...
        protected Texture2D LoadIcon(string iconName)
        {
            // Сначала попробуем загрузить из Resources
            var icon = Resources.Load(iconName) as Texture2D;
            if (icon != null) return icon;
            
            // Если не найдено в Resources, попробуем загрузить из Textures
            var texturePath = "Assets/Invector-3rdPersonController/Basic Locomotion/Textures/" + iconName + ".png";
            icon = UnityEditor.AssetDatabase.LoadAssetAtPath<Texture2D>(texturePath);
            if (icon != null) return icon;
            
            // Если ничего не найдено, вернем дефолтную иконку
            return Resources.Load("icon_v2") as Texture2D;
        }
        ...
        public GUIContent[] getToobarTitles()
        {
            ...
            {               
                ...
                {
                    Texture icon = null;
                    if (!string.IsNullOrEmpty(toolbars[i].iconName))
                    {
                        var _icon = LoadIcon(toolbars[i].iconName);
