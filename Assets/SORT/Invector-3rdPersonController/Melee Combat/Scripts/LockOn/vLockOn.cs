...
namespace Invector.vCharacterController
{
    ...
    public class vLockOn : vLockOnBehaviour
    {
        ...
        [Tooltip("Расстояние камеры при lock on")]
        public float lockOnCameraDistance = 2f; // УМЕНЬШИЛ ДО 2!
        
        [Tooltip("Принудительно поворачивать игрока к врагу")]
        public bool forcePlayerRotation = true;
        
        [Tooltip("Скорость поворота игрока")]
        public float rotationSpeed = 15f; // УВЕЛИЧИЛ СКОРОСТЬ!
        ...
        protected vThirdPersonMotor tpMotor;
        protected float originalCameraDistance;
        protected bool originalDistanceSaved = false;
        ...
        protected virtual void Start()
        {
            ...
            tpCamera = vThirdPersonCamera.instance;
        ...
        protected virtual void UpdateLockOn(vThirdPersonInput tpInput)
        {
            ...
            // ПРИНУДИТЕЛЬНЫЙ ПОВОРОТ ИГРОКА К ВРАГУ!
            if (forcePlayerRotation && isLockingOn && currentTarget != null)
            {
                ForcePlayerRotationToTarget();
            }
        }
        ...
        /// <summary>
        /// ПРИНУДИТЕЛЬНО ПОВОРАЧИВАЕТ ИГРОКА К ВРАГУ!
        /// </summary>
        protected virtual void ForcePlayerRotationToTarget()
        {
            if (currentTarget == null || tpMotor == null) return;

            // Вычисляем направление к врагу
            Vector3 directionToTarget = (currentTarget.position - transform.position).normalized;
            directionToTarget.y = 0; // Игнорируем Y компонент
            
            if (directionToTarget != Vector3.zero)
            {
                // ПРИНУДИТЕЛЬНО ПОВОРАЧИВАЕМ ИГРОКА К ВРАГУ!
        ...
        protected override void SetTarget()
        {
            ...
            {
                // Сохраняем оригинальное расстояние камеры при первом lock on
                if (!originalDistanceSaved && tpCamera != null)
                {
                    originalCameraDistance = tpCamera.currentState.defaultDistance;
                    originalDistanceSaved = true;
                }
                
                tpInput.tpCamera.SetLockTarget(currentTarget.transform, cameraHeightOffset);
                onLockOnTarget.Invoke(currentTarget);
                
                // ПРИБЛИЖАЕМ КАМЕРУ НАПОЛОВИНУ!
                if (tpCamera != null)
                {
                    tpCamera.currentState.defaultDistance = lockOnCameraDistance;
                    tpCamera.currentState.minDistance = lockOnCameraDistance;
                    tpCamera.currentState.maxDistance = lockOnCameraDistance;
                }
            }
        }
        ...
        public virtual void StopLockOn()
        {
            if (currentTarget == null && tpInput.tpCamera != null)
            {
                ...
                // Восстанавливаем оригинальное расстояние камеры
                if (tpCamera != null && originalDistanceSaved)
                {
                    tpCamera.currentState.defaultDistance = originalCameraDistance;
                    tpCamera.currentState.minDistance = 1f;
                    tpCamera.currentState.maxDistance = 10f;
                }
            }
        }       
