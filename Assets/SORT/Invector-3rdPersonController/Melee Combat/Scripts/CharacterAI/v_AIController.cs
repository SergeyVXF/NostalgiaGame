...
{   
    ...
    public class v_AIController : v_AIAnimator, vIMeleeFighter
    {
        ...
        IEnumerator ResetAttackCount()
        {
            ...
            // Убираем задержку - мгновенно восстанавливаем атаку
            yield return null; // Один кадр вместо секунд
        ...
        {
            // Мгновенно прерываем текущую анимацию
            if (animator != null)
            {
        ...
        {
            // Мгновенно прерываем текущую атаку при получении урона
            if (isAttacking)
            {
                // Прерываем атаку
                isAttacking = false;
                ...
                // Принудительно переключаем на hit reaction
                if (animator != null && animator.enabled)
                {
                    // Устанавливаем ID реакции
                    animator.SetInteger("RecoilID", damage.recoil_id);
                    
                    // Запускаем hit reaction
            ...
            // Уменьшенная заморозка врага при ударе игрока
            if (!isFrozen && attacker != null && attacker.gameObject.CompareTag("Player"))
            {
                Freeze(0.1f); // Уменьшаем время заморозки с 0.3f до 0.1f
            }
