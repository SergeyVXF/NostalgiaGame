...
{     
    public class v_AIAnimator : v_AIMotor
    {
        ...
        void OnAnimatorMove()
        {
            ...
            if (!animator || !agent || !_rigidbody) return;
        ...
        void LocomotionAnimation(float _speed, float _direction)
        {
            ...
            isGrounded = agent && agent.enabled ? agent.isOnNavMesh : isRolling ? true : groundDistance <= groundCheckDistance;
        ...
        void DeadAnimation()
        {
            if (!isDead || !animator || !animator.enabled) return;
        ...
        {
            if (!animator || !animator.enabled || !_rigidbody || !agent) return;
        ...
        {
            if (!animator || !animator.enabled || !meleeManager) return;
