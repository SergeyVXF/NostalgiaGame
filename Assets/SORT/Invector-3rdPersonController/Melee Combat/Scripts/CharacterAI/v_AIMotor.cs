...
namespace Invector.vCharacterController.AI
{
    ...
    public class v_AIMotor : vCharacter
    {
        ...
        [Tooltip("Enable/disable automatic crouching when AI detects obstacles above head")]
        public bool aiUseAutoCrouch = true;
        [Tooltip("Delay between auto crouch actions (seconds)")]
        [Range(0.1f, 5f)]
        public float aiAutoCrouchDelay = 1f;
        ...
        protected float lastAutoCrouchTime;
        ...
        {
            // Если auto crouch отключен, не выполняем проверку
            if (!aiUseAutoCrouch)
            {
                isCrouched = false;
                return;
            ...
            // Проверяем задержку между приседаниями
            if (Time.time - lastAutoCrouchTime < aiAutoCrouchDelay)
            {
            ...
            {
                isCrouched = true;
                lastAutoCrouchTime = Time.time; // Записываем время последнего приседания
            }
            else
            {
                isCrouched = false;
