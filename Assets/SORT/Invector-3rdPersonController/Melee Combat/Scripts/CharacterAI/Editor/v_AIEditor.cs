...
namespace Invector.vCharacterController.AI
{
    ...
    public class v_AIEditor : vEditorBase
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            // Добавляем секцию для Auto Crouch
            EditorGUILayout.Space();
            EditorGUILayout.LabelField("Auto Crouch Settings", EditorStyles.boldLabel);
            
            SerializedProperty useAutoCrouchProp = serializedObject.FindProperty("aiUseAutoCrouch");
            SerializedProperty autoCrouchDelayProp = serializedObject.FindProperty("aiAutoCrouchDelay");
            
            if (useAutoCrouchProp != null)
            {
                EditorGUILayout.PropertyField(useAutoCrouchProp, new GUIContent("AI Use Auto Crouch", "Enable/disable automatic crouching when AI detects obstacles above head"));
                
                if (useAutoCrouchProp.boolValue && autoCrouchDelayProp != null)
                {
                    EditorGUILayout.PropertyField(autoCrouchDelayProp, new GUIContent("Auto Crouch Delay", "Delay between auto crouch actions (seconds)"));
                }
                
                if (!useAutoCrouchProp.boolValue)
                {
                    EditorGUILayout.HelpBox("AI Auto Crouch отключен - AI не будет приседать при обнаружении препятствий", MessageType.Info);
                }
                else
                {
                    EditorGUILayout.HelpBox("AI Auto Crouch включен - AI будет приседать с задержкой между действиями", MessageType.Info);
                }
            }
